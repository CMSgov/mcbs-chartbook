...
_NEWRECS=_RECSPM=_DIFPMS AND _RECSPM NE _PURCHPM;
SOB='450';
...
PROC UNIVARIATE; VAR HDPRVI; TITLE 'FIRST';run; title;
...
DATA PART450(DROP=TMPMUL TMPAAA);
SET A;
...
IF TMPNS NE . THEN MYHDPRVI=HDPRVI/_PURCHPM;
...
IF MULTIPLE=TMPMULT THEN  DO; 
    TMPEV=TMPEV+EVPRICE;
    TMPEV2=TMPEV2+AVGEVPR;
END;

IF FIRST.MULTIPLE THEN 
DO;
  TMPEV=EVPRICE;
  TMPEV2=AVGEVPR;
  TMPMULT=MULTIPLE;
...
HDPRVI=ROUND(HDPRVI,.01);
AMTPRVI_=ROUND(AMTPRVI_,.01);
HDPRVI_=ROUND(HDPRVI_,.01);
...
AND TOTALCHG NE . AND TOTALCHG>0 AND TMPNS NE . THEN DO;
 ...
 IF HDPRVI=HDPRVI_ AND _SOPPRVI=1 THEN HDPRVI=HDPRVI_/_PURCHPM;
...
PROC UNIVARIATE; VAR HDPRVI; TITLE 'SECOND';RUN;title;
