...
IF _NEWRECS=_RECSPM=_DIFPMS AND _NEWRECS NE . AND _PURCHPM=.;
...
MULTIPLE=BASEID||_NEWRECS||_RECSPM||_DIFPMS||_PURCHPM||TOTALCHG;
RUN;


DATA DRUGS702b;
SET MCBSDATA.OUTCOMM_Y&CurrYear;
IF _NEWRECS=_RECSPM AND _NEWRECS NE . AND _PURCHPM=. AND _NEWRECS NE _DIFPMS;
IF _SOPCARE=1 THEN DELETE;
SOB='702';
PMROID=BASEID||EVNTNUM||ROUND;
ID=BASEID||EVNTNUM||ROUND||UTILNUM;
...
RUN;
DATA C(KEEP=MULTIPLE TMPNS COUNTER);
SET B;
BY MULTIPLE;
IF LAST.MULTIPLE;
RUN;
