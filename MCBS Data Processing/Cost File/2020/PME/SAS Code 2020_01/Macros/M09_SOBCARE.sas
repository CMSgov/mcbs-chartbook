...
| 100=_NEWRECS=1_RECSPM  DIFPMS =1 URCHPM=1                 |
...
IF _SOPCARE=1;
IF _NEWRECS>_RECSPM THEN DELETE;
IF _NEWRECS=. THEN DELETE;
SOB='CARE';
...
IF FIN_CHG = . AND PAYFACT='M' THEN DO;BESTEST=(EVPRICE*&M_factor);END;
...
RETAIN TMPMUL '';
IF TRY=. THEN DELETE;
IF FIRST.MULTIPLE THEN DO; TMPMUL=MULTIPLE; END;
RUN;
...
IF PERCENT2 NE . AND TOTALCHG>0 AND TMPNS NE . THEN DO;
 ...
 IF PAYFACT='M' THEN _AMTOOP=AMTOOP_/_PURCHPM;
 IF PAYFACT='P' THEN _AMTOOP=AMTOOP_/_PURCHPM;
 IF PAYFACT='A' THEN _AMTOOP=AMTOOP_/_PURCHPM;
 IF PAYFACT NE 'C' AND PAYFACT NE 'H' AND PAYFACT NE 'V' AND PAYFACT NE 'M'
...
IF PERCENT2 NE . AND TOTALCHG<0 AND TMPNS NE . THEN DO;
 ...
 IF PAYFACT='M' THEN _AMTOOP=AMTOOP_/PURCHPM_;
...
IF PERCENT2 NE . AND TOTALCHG>0 AND TMPNS=. THEN DO;
 ...
 IF PAYFACT='M' THEN _AMTOOP=AMTOOP_;
