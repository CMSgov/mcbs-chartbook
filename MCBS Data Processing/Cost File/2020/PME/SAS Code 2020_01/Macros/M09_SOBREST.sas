...
DATA THEYIN;
SET 
MCBSDATA.SOB100(KEEP=BASEID ID)
MCBSDATA.SOB200(KEEP=BASEID ID)
MCBSDATA.SOB300(KEEP=BASEID ID)
MCBSDATA.SOB400(KEEP=BASEID ID)
MCBSDATA.SOB450(KEEP=BASEID ID)
MCBSDATA.SOB500(KEEP=BASEID ID)
MCBSDATA.SOBOTH(KEEP=BASEID ID)
MCBSDATA.SOBCARE(KEEP=BASEID  ID)
MCBSDATA.SOBMISS(KEEP=BASEID  ID)
;
PROC SORT DATA = THEYIN NODUPKEY;
BY ID ;
PROC SORT DATA=MCBSDATA.OUTCOMM_Y&CurrYear OUT=ALL;
BY ID;
DATA NOTOG TOG;
MERGE ALL (IN=A) THEYIN (IN=B);
BY ID;
IF A;
