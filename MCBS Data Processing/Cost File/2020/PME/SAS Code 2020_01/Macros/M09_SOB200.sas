...
| 100=_RECSEV=1_RECSPM=1_DIFPMS=1_PURCHPM=1                 |
| 200=_RECSEV=1 _RECSPM=1 _DIFPMS=1 _PURCHPM>1              |
| 300=_RECSEV>1=_RECSPM>1 _DIFPMS=1 _PURCHPM>1              |
|400=_RECSEV>1=_RECSPM>1=_DIFPMS>1=_PURCHPM>1               |
|450=_RECSEV>1=_RECSPM>1=_DIFPMS>1 _PURCHPM>1 AND NE _RECSEV|
| 500=_RECSEV>_RECSPM                                       |
| 600=_RECSEV=_RECSPM=_PURCHPM AND NE _DIFPMS               |
| 700=_RECSEV=_RECSPM=_DIFPMS _PURCHPM=MISSING              |
| 800=ALL ARE MISSING                                       |
|-----------------------------------------------------------|
;
/*
...
_DIFPMS,
_PURCHPM,
COUNT(BASEID) AS COUNT 
FROM MCBSDATA.OUTCOMM_Y&CurrYear
GROUP BY _NEWRECS,
_RECSPM,
_DIFPMS,
_PURCHPM
...
IF _TREIM = . AND EVPRICE=. AND PAYFACT='C' AND TMPSET=0 THEN DO;_AMTOOP=AMTOOP_;END;
IF _TREIM = . AND EVPRICE=. AND PAYFACT='H' AND TMPSET=1 THEN DO;_AMTOOP=AMTOOP_/NUMLINKS;END;
IF _TREIM = . AND EVPRICE=. AND PAYFACT='H' AND TMPSET=0 THEN DO;_AMTOOP=AMTOOP_;END;
IF _TREIM = . AND EVPRICE=. AND PAYFACT='V' AND TMPSET=1 THEN DO;_AMTOOP=AMTOOP_/NUMLINKS;END;
IF _TREIM = . AND EVPRICE=. AND PAYFACT='V' AND TMPSET=0 THEN DO;_AMTOOP=AMTOOP_;END;
...
IF _SOPOOP=1 THEN 
 DO; 
   IF TMPSET=0 THEN _AMTOOP=AMTOOP_;
   IF TMPSET=1 THEN _AMTOOP=AMTOOP_/NUMLINKS;
 END;
IF _SOPDISC NE 0 THEN _AMTDISC=.;
IF _SOPOTH NE 0 THEN _AMTOTH=.;
IF _SOPPD NE 0 THEN _AMTPD=.;
